#!/usr/bin/env bash
# claude-vault — immediate backup when a durable file is WRITTEN, mid-response.
#
...
source "${CLAUDE_VAULT_CONFIG:-$HOME/.config/claude-vault/config.sh}" 2>/dev/null || exit 0
